...
UNION select * from score where degree=86
union select * from score where degree=88
...
create table grade(low   int,upp   number(3),rank   char(1));
...
#12、查询Score表中至少有5名学生选修的并以3开头的课程的平均分数。
SELECT AVG(degree) FROM score GROUP BY cno HAVING COUNT(sno) >= 5
AND cno LIKE '3%'
#13、查询最低分大于70，最高分小于90的Sno列。
...
COMMIT;
#现查询所有同学的Sno、Cno和rank列。
SELECT o.sno,o.cno,g.rank
FROM score o,grade g
...
WHERE YEAR(a.sbirthday) = YEAR(b.sbirthday)
AND b.sno = '108' 
AND a.sno != '108'
...
FROM
((
SELECT tname AS ctname,prof AS cprof
FROM teacher
WHERE depart = '计算机系'
) a,
(
SELECT tname AS etname,prof AS eprof
FROM teacher
WHERE depart = '电子工程系'
) b)
WHERE a.cprof != b.eprof
...
#38、查询Student表中每个学生的姓名和年龄。
SELECT sname,2019 - YEAR(sbirthday)
FROM student
#39、查询Student表中最大和最小的Sbirthday日期值。
